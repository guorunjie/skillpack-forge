...
const MCP_SERVER_TOOLS = MCP_TOOLS.map((tool) => ({
  ...tool,
...
function mcpServerName(manifest) {
  return `${manifest.name}-skillpack`;
}
...
const tools = ${jsonForScript(MCP_SERVER_TOOLS)};
  ...
  try {
    if (method === "initialize") {
      ok(id, {
        ...
        serverInfo: {
          name: ${JSON.stringify(mcpServerName(manifest))},
          version: "1.0.0"
        }
      });
      return;
...
function renderMcpManifest(manifest) {
  const bundleName = mcpServerName(manifest);
  return `${JSON.stringify(
    {
      manifest_version: "0.3",
      name: bundleName,
      display_name: `${manifest.name} Skillpack MCP Server`,
      version: "1.0.0",
      description: manifest.summary,
      long_description:
        "Generated by Skillpack Forge from skillpack.yaml. This read-only local MCP server exposes project context, commands, and workflows for AI coding agents.",
      author: {
        name: manifest.name
      },
      server: {
      ...
      tools: MCP_TOOLS,
      keywords: ["skillpack", "mcp", "ai-agents", "repo-context", "developer-tools"],
      compatibility: {
      ...
      _meta: {
        "io.github.guorunjie.skillpack-forge": {
          generated_from: "skillpack.yaml",
...
If your MCP client supports \`${manifest.name}-skillpack.mcpb\` installation, use the bundle path from the packaging step instead of wiring the stdio server manually.
...
\`\`\`bash
skillpack-forge mcpb . ${manifest.name}-skillpack.mcpb
\`\`\`
...
function generatedArtifacts(manifest) {
  ...
  if (targets.has("mcp")) {
    artifacts.push({ file: ".mcp/manifest.json", content: renderMcpManifest(manifest) });
...
async function generatedFilesOnDisk(root) {
  const candidates = [
    ...
    ...(await collectMatchingFiles(root, ".mcp", (file) => file.endsWith(".mjs") || file.endsWith(".md") || file.endsWith(".json"))),
